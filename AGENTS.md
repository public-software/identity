...
This is `identity`, one repository of [Public Software](https://publicsoftware.dev): a spec-first cleanroom Rust suite built by people and coding agents together. Read this file before the README; it is the contract every agent works under here.

**Ring:** platform · **Layers:** L6 · **Wave:** 1 · IdP (Kanidm-derived), OIDC/passkeys client, account picker, secrets vault client, attestation verifier (later), digital ID wallet core. Planned components: pubd-idp · pub-identity-client · pub-vault · pub-attest (wave 4) · pub-wallet.
...
- A new crate is `pub new <kind> <name>` with a kind from lib, app, service, plugin, spec; it lands in `crates/pub-identity-<name>/` and gets a `[[component]]` entry in `CATALOG.toml`.

## Conventions

- Crates are named `pub-identity-<name>`; a service is the binary `pubd-<name>`; WIT interfaces live under `public:`.
...
- The merge gates decide (`pub check`, CI, the agent review `suite / policy`, the maintainer team `public-software/maint-identity`), not who or what wrote the change.
<!-- agents:end -->

## Repo notes

_Maintainers of `identity` write here what the generated section cannot know: how the crates fit together, how to run the thing, which specifications matter, what not to touch. Everything above this heading is regenerated._
